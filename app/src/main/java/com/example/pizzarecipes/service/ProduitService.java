...
public class ProduitService implements IDao<Produit> {
    ...
    private static ProduitService essInstance;
    private final List<Produit> essCatalogue = new ArrayList<>();

    private ProduitService() {
        essInitialiserPizzas();
    }

    public static ProduitService getInstance() {
        if (essInstance == null) {
            essInstance = new ProduitService();
        }
        return essInstance;
    }

    private void essInitialiserPizzas() {

        create(new Produit(
                "Pizza Poulet Barbecue Maison",
                3,
                R.mipmap.pizza1,
                "35 min",
        ...
        create(new Produit(
                "Pizza Rustique Façon Deep Dish",
                8,
                R.mipmap.pizza4,
                "45 min",
        ...
        create(new Produit(
                "Pizza Farcie Pepperoni Saucisse",
                5,
                R.mipmap.pizza10,
                "45 min",
                "- 1 package (1/4 ounce) active dry yeast\n" +
    ...
    @Override
    public Produit create(Produit essElement) {
        essCatalogue.add(essElement);
        return essElement;
    }

    @Override
    public Produit update(Produit essElement) {
        for (int essIndex = 0; essIndex < essCatalogue.size(); essIndex++) {
            if (essCatalogue.get(essIndex).getEssIdentifiant() == essElement.getEssIdentifiant()) {
                essCatalogue.set(essIndex, essElement);
                return essElement;
            }
        }
        return null;
    ...
    @Override
    public boolean delete(long essId) {
        return essCatalogue.removeIf(essProduit -> essProduit.getEssIdentifiant() == essId);
    }

    @Override
    public Produit findById(long essId) {
        for (Produit essProduit : essCatalogue) {
            if (essProduit.getEssIdentifiant() == essId) {
                return essProduit;
            }
        }
        return null;
    ...
    public List<Produit> findAll() {
        return Collections.unmodifiableList(essCatalogue);
    }
}